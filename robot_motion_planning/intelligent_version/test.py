...
if __name__ == '__main__':
    ...
    column_names = ['maze', 'mode','num_actions_1', 'length_movement_1', 'num_actions_2', 'length_movement_2', 'true_coverage', 'score']
    ...
    for mode in modes:
        for i in xrange(num_tests):
            print('Simulation: %d. Mode: %s.' %(i, mode))
            ...
            df = df.append({column_names[0]: (str(sys.argv[1]).split('.')[0]).split('_')[-1], column_names[1]:mode, column_names[2]:result[0], column_names[3]:result[1], column_names[4]:result[2], column_names[5]:result[3], column_names[6]:result[4], column_names[7]:result[5]}, ignore_index=True)
            # Print result
